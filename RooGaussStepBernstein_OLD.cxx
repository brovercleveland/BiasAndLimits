...
namespace {
  double zeroth(const double x, const double m, const double s, 
    ...
    return ( std::sqrt(M_PI/2.0)*s*
       ( TMath::Erf(erf_parm_b) - TMath::Erf(erf_parm_a) ));
  }
  double zeroth_integral(const double x, const double m, const double s,
    ...
    const double erf_a = TMath::Erf((a+m-x)/(root2*s));
    const double erf_b = TMath::Erf((b+m-x)/(root2*s));
  ...
  double first(const double x, const double m, const double s,
    ...
    return ( s*s*(gaus_b-gaus_a) + 
       rootpiover2*s*(a+m-x)*
       (TMath::Erf(erf_parm_b)-TMath::Erf(erf_parm_a)) )/(a-b);
  }
...
gaus*rootpiover2*poly3*TMath::Erf(erf_parm) ) );
  }
