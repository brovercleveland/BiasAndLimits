...
dataVals = [175.0,460.0,750.0,990.0,1365.0,1580.0,1615.0,1580.0,1570.0,1490.0,1425.0,1320.0,1170.0,1180.0,1040.0,1035.0,930.0,890.0,880.0,770.0,710.0,
    660.0,645.0,555.0,570.0,530.0,500.0,445.0,450.0,440.0,385.0,390.0,360.0,365.0,280.0,320.0,275.0,275.0,240.0,235.0,240.0,250.0,205.0,200.0,215.0]
...
# make plots for 2011, 2012, 2011+2012, various binnings
  ...
  leg1 = TLegend(0.45, 0.66, 0.75, 0.84)
  leg2 = TLegend(0.75, 0.45, 0.95, 0.58)
  ...
  for year in dataDict.keys():
    for width in binningDict.keys():
      ...
      for i in range(0,45):
        #print 'dataVal:', dataDict[year].getVal(i)
        print 'pdfVal:', pdfDict[year].getVal()
      ...
      pad1 = TPad('pad1', '', 0.02, 0.30, 0.90, 0.98, 0)
...
#test the error bands
      if year == 'All' and width == '2 GeV':
        ...
        doBandsNew(onesigma, twosigma, mzgList, pdfDict[year], bgPdfList, bgNormList, dataDict[year], frame, fitres, tmpCurve, frac_ALLunweightPdfList,1,xRatio,yRatio,yErrRatio)
      ...
      frame.GetXaxis().SetTickLength(gStyle.GetTickLength()/2)
      #frame.GetXaxis().SetNdivisions(910)
      ...
      L1.SetTextFont(62)
      L1.DrawLatex(0.40, 0.955, "#sqrt{s} = 7 TeV, L = 5 fb^{-1} #sqrt{s} = 8 TeV, L = 19.6 fb^{-1}");
      chan1.SetNDC()
      chan1.SetTextSize(0.049)
      ...
      pad2.cd()
      pad2.SetGrid(1,1)
      frameR = pad2.DrawFrame(100,0.75,190,1.28)
      frameR.GetYaxis().SetLabelSize(0.09)
      frameR.GetYaxis().SetTitle('Data/Background')
      frameR.GetYaxis().SetTitleSize(0.09)
      frameR.GetYaxis().SetTitleOffset(0.5);
      frameR.GetYaxis().SetNdivisions(5);
      frameR.GetYaxis().CenterTitle();
      frameR.GetXaxis().SetLabelSize(0.1);
      frameR.GetXaxis().SetTitleSize(0.1);
      frameR.GetXaxis().SetTitle('m_{ll#gamma} (GeV)')
      frameR.GetXaxis().SetTitleOffset(1.00);
      frameR.Draw()

      xErrRatio = [0]*45
      hRatio = TGraphErrors(45,np.array(xRatio), np.array(yRatio), np.array(xErrRatio), np.array(yErrRatio))
...
#make a scale to renormalize the weighted sets to the original yield
      ...
      if year == 'All' and width == '2 GeV':
        ...
        #pdfDict[year].plotOn(frame, RooFit.Range('oldRegion'),RooFit.VisualizeError(fit_s))
        #doBandsFit(onesigma, twosigma, mzg_muon_2012, pdfDict[year], tmpCurve, dataDict[year], frame)
        doBandsNew(onesigma, twosigma, mzgList, pdfDict[year], bgPdfList, bgNormList, dataDict[year], frame, fitres, tmpCurve, frac_ALLweightPdfList, scale, xRatio, yRatio, yErrRatio)
        twosigma.SetLineColor(kYellow)
        twosigma.SetFillColor(kYellow)
        twosigma.SetMarkerColor(kYellow)
...
def doBandsNew(onesigma, twosigma, hmassList, cpdf, pdfList, normList, datanorm, plot, fit_s, nomcurve, weightList, weightScale, ratioX,ratioY,ratioYerr):
  if verbose:
    print 'fit_s'
    fit_s.Print()
  for ibin in range(1,plot.GetXaxis().GetNbins()+1):
    ...
    if verbose:
      print 'final numbers:'
      print 'dataVal:',dataVals[ibin-1]

      print 'nombkg:', nombkg
      print 'normerr:', normerr
      raw_input('Hit any key to continue')
    ratioX.append((plot.GetXaxis().GetBinLowEdge(ibin)+plot.GetXaxis().GetBinUpEdge(ibin))/2)
    ratioY.append(dataVals[ibin-1]/nombkg)
    ratioYerr.append((dataVals[ibin-1]/nombkg)*sqrt((normerr/nombkg)**2+(sqrt(dataVals[ibin-1])/dataVals[ibin-1])**2))
    print ratioYerr[-1]
    #normerr = 1.0
